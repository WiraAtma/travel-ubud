...
<div class="max-w-7xl mx-auto px-4 py-10">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between mb-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-800">Kelola Hotel</h1>
                <p class="text-gray-500 mt-1">
                    {{ request()->routeIs('hotels.all') ? 'Daftar semua hotel dari seluruh pengguna' : 'Daftar hotel yang Anda kelola' }}
                </p>
            </div>
        </div>
    ...
    <div class="flex items-center gap-2 mb-6">
        <a href="{{ route('hotels.index') }}"
           class="px-4 py-1.5 rounded-full text-sm font-medium transition-colors duration-150
                  {{ request()->routeIs('hotels.index') ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50' }}">
            Hotel Saya
        </a>
        <a href="{{ route('hotels.all') }}"
           class="px-4 py-1.5 rounded-full text-sm font-medium transition-colors duration-150
                  {{ request()->routeIs('hotels.all') ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50' }}">
            Semua Hotel
        </a>
    </div>
    @endif
        ...
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            @if ($hotels->total() > 0)
                <span class="text-sm text-gray-500">
                    Menampilkan
                    <span class="font-semibold text-gray-700">{{ $hotels->firstItem() }}</span>
                    –
                    <span class="font-semibold text-gray-700">{{ $hotels->lastItem() }}</span>
                    dari
                    <span class="font-semibold text-gray-700">{{ $hotels->total() }}</span>
                    hotel
                </span>
                <span class="text-sm text-gray-400">Halaman {{ $hotels->currentPage() }} / {{ $hotels->lastPage() }}</span>
            @else
                <span class="text-sm text-gray-400">Belum ada hotel</span>
            @endif
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50 text-gray-500 uppercase text-xs tracking-wider">
                        <tr>
                            ...
                            <th class="px-6 py-4 font-medium">Nama Hotel</th>
                            <th class="px-6 py-4 font-medium">Alamat</th>
                            <th class="px-6 py-4 font-medium">Harga Mulai</th>
                            <th class="px-6 py-4 font-medium">Check-in / Out</th>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4">
                                @if ($hotel->image_cover)
                                    <img src="{{ Storage::disk('supabase')->url($hotel->image_cover) }}"
                                         alt="cover"
                            ...
                            {{-- Nama --}}
                            <td class="px-6 py-4">
                                <p class="font-medium text-gray-800 line-clamp-1 max-w-[180px]">{{ $hotel->name }}</p>
                                <p class="text-xs text-gray-400 mt-0.5">{{ $hotel->phone }}</p>
                            </td>

                            {{-- Alamat --}}
                            <td class="px-6 py-4 text-gray-600 text-xs max-w-[160px]">
                                <p class="line-clamp-2">{{ $hotel->address }}</p>
                            </td>
                            ...
                            <td class="px-6 py-4 text-gray-600 text-xs whitespace-nowrap">
                                <div class="flex flex-col gap-0.5">
                                    <span><span class="text-green-600 font-medium">In</span> {{ \Carbon\Carbon::parse($hotel->checkin_time)->format('H:i') }}</span>
                                    <span><span class="text-red-500 font-medium">Out</span> {{ \Carbon\Carbon::parse($hotel->checkout_time)->format('H:i') }}</span>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-1">
                                    ...
                                    <span class="text-sm font-medium text-gray-700">{{ number_format($hotel->rating, 1) }}</span>
                                    <span class="text-xs text-gray-400">({{ $hotel->rating_count }})</span>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <div class="w-7 h-7 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center text-xs font-semibold uppercase flex-shrink-0">
                                        {{ substr($hotel->author->name ?? '?', 0, 1) }}
                                    </div>
                                    <span class="text-gray-600 text-xs">{{ $hotel->author->name ?? '-' }}</span>
                                </div>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-6 py-16 text-center text-gray-400">
                                <p class="text-sm">Belum ada hotel.
                                    <a href="{{ route('hotels.create') }}" class="text-indigo-500 hover:underline">Tambah sekarang</a>
                                </p>
                            </td>
                        </tr>
                    @endforelse
