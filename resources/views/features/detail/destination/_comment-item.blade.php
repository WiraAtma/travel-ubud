    ...
    <div class="flex-1 min-w-0">
        ...
        <div class="flex items-center gap-3 mt-2 flex-wrap">
            @auth
                ...
                @if ($isOwner || $isAdmin)
                    <button type="button"
                            onclick="confirmDelete('{{ route('destination.comments.destroy', $comment->id) }}')"
        ...
        @if ($isOwner)
            <div id="edit-form-{{ $comment->id }}" class="hidden mt-3">
                <form method="POST" action="{{ route('destination.comments.update', $comment->id) }}">
                    @csrf @method('PUT')
        ...
        @auth
            @if ($depth === 0)
                <div id="reply-form-{{ $comment->id }}" class="hidden mt-3">
                    <form method="POST" action="{{ route('destination.comments.store', $destination->id) }}">
                        @csrf
